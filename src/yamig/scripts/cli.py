...
class YamigCLI:
    ...
    def _parse_output_path(self, args: Namespace):
        if args.output_path is None:
            args.output_path = Path(f'{args.input_path.stem}.msch').absolute()
        
        if args.output_path.exists():
            if not args.output_path.is_file():
                raise FileExistsError(f'{args.output_path} exists and it\'s not a file!')
                ...
                if overwrite:
                    ...
                    raise FileExistsError(f'{args.output_path} exists!')


    def _parse_resolution(self, args: Namespace):
        ...
        if suffix == 'px':
            ...
            self.logger.warning('tile-logic-display is only supported display for now! using 32px multiplier')
    ...
    def run(self) -> None:
        ...
        params = YamigParams(
            ...
            display=Content.TILE_LOGIC_DISPLAY, #! remove this
            processor=Content.MICRO_PROCESSOR, #! remove this
